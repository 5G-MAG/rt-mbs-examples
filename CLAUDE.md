...
- `scripts/tmux/` — tmux scripts that start all processes needed for the MBSF/MBSTF tutorials (Open5GS NFs, MBSTF, MBSF, mock media server) in one session. Machine-specific paths (`OPEN5GS_BASE_DIR`, `MBSF_BASE_DIR`, etc.) default to checkouts in `$HOME` and are overridden per user in the gitignored `scripts/tmux/local.env` (template: `local.env.example`) — never hardcode personal paths in the scripts themselves. They depend on sibling repos (`open5gs`, `rt-mbs-function`, `rt-mbs-transport-function`) being built locally.
